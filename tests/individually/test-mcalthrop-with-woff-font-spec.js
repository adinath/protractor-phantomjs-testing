...
describe('simple protractor test', function () {
    ...
    beforeEach(function () {
        location = 'http://mcalthrop.github.io/protractor-phantomjs-testing/index-with-woff-font.html';
    });

    it('mcalthrop (with all WOFF font): should get text from named id', function () {
        console.log('Browsing', location);
        browser.get(location);
        expect(element(by.id('test-id')).getText()).toEqual('test text');
    });
    it('mcalthrop (with all WOFF font): should pass basic test of data binding', function () {
        var name = 'Bruce';
